...
export function handleInboundMessage(connection: ClientConnection, raw: string): void {
  ...
  switch (message.type) {
    ...
    case "FREQUENCY_UPDATE": if (!requireAuth(connection)) return; broadcast(encodeMessage(message), connection); break;
    case "PTT_START": case "PTT_STOP": case "VOICE_METADATA": if (!requireAuth(connection)) return; broadcast(encodeMessage(message), connection); break;
